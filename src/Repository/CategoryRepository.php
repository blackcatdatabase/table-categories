...
use BlackCat\Database\Packages\Categories\Repository\CategoryRepositoryInterface;
...
class CategoryRepository implements CategoryRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\Categories\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'name', 'parent_id', 'updated_at', 'version', 'deleted_at' ]);
          return count($rows);
        }
