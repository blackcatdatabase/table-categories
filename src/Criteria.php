 ...
 * - whitelist filtrů: [ 'id', 'name', 'slug', 'parent_id', 'created_at', 'updated_at', 'deleted_at' ]
 * - whitelist pro LIKE hledání: [ 'name', 'slug' ]
 */
final class Criteria {
    ...
    public function toSql(bool $viewMode = false): array {
        ...
        // filtry (rovnosti / IN)
        $allowed = array_fill_keys([ 'id', 'name', 'slug', 'parent_id', 'created_at', 'updated_at', 'deleted_at' ], true);
        ...
        $order = null;
        if ($this->sort) {
            $safeCols = array_fill_keys([ 'id', 'name', 'slug', 'parent_id', 'created_at', 'updated_at', 'deleted_at' ], true);
            $parts = [];
