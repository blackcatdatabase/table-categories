...
namespace BlackCat\Database\Packages\Categories\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\Categories\Dto\CategoryDto;
use BlackCat\Database\Packages\Categories\Mapper\CategoryDtoMapper;
use BlackCat\Database\Packages\Categories\Repository\CategoryRepository;
 ...
 */
final class CategoriesAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private CategoryRepository $categoryRepo
    ) {
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\Categories\ModuleException("Record {$idStr} not found for locking.");
            }
